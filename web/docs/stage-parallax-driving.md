...
In [`ParallaxDrivePrototype`](../src/ui/ParallaxDrivePrototype.tsx), the absolute ladder strips **`bg-5` … `fg-2`** use a signed **`pitch`** in `[-1, 1]` mapped through [`computeShutterLadderRects`](../src/ui/parallaxShutterLayout.ts). The **vertical band** is fixed in reference px between the **BG6 bottom seam** and **FG1 top seam** (same fractions as [`index.css`](../src/index.css) flex column + inner FG1 `60%` slice; see [`parallaxShutterLayout.ts`](../src/ui/parallaxShutterLayout.ts)). **Blues** (`bg-5`–`bg-1`) and **greens** (`fg-6`–`fg-2`) **split that band height**: at `pitch = 0` the split is **50/50** (`NEUTRAL_BLUE_HEIGHT_SHARE`); at `pitch → +1` (downhill) blues take the full band and green strip heights go to **0**; at `pitch → −1` (uphill) the reverse. Strip **positions and heights** come from that model (not per-rung offset ramps). **Drag down** on the stage → downhill (`pitch > 0`); **drag up** → uphill (`pitch < 0`). **Releasing the pointer** eases `pitch` back to **0** (spring). Toggle **`PARALLAX_HORIZON_DRAG_ENABLED`** in the same file to disable the drag layer.

A signed **`lateral`** in `[-1, 1]` is driven from the **same** drag layer: **drag left** → positive `lateral`, **drag right** → negative `lateral` (inverted screen X). All six BG layers use **`lateralTranslateRefPx`** in [`parallaxPrototypeGeometry.ts`](../src/ui/parallaxPrototypeGeometry.ts): motion is **`(REFERENCE_WIDTH / 2) * (depthFrac / 0.9)`** at `|lateral| = 1`, so **`bg-1`** reaches the stage left/right center line and **`bg-6` … `bg-2`** keep the **same relative speeds** as the legacy depth table (e.g. `bg-6` at one-third of `bg-1`). **BG ladder strips** (`bg-5`–`bg-1`) and the **BG6 wide band** use **2× reference width** (`PARALLAX_STRIP_WIDTH_REF_PX`) with symmetric horizontal slack so lateral translation never exposes a seam. **All FG layers** (`fg-1`, `fg-6`–`fg-2`) do not translate laterally; they share `REFERENCE_WIDTH` at `xPx = 0` and are absolutely positioned from `PARALLAX_PROTOTYPE_LAYER_POSITION_DEFAULT` / `SIZE_DEFAULT` in [`parallaxPrototypeLayerLayout.ts`](../src/ui/parallaxPrototypeLayerLayout.ts) (FG ladder strip widths come from `GREEN_STRIP_WIDTH_PX` / `GREEN_STRIP_X_PX` in [`parallaxShutterLayout.ts`](../src/ui/parallaxShutterLayout.ts)). **Release** eases **`lateral`** to **0** with the **same** spring as `pitch`. Each BG layer includes a **temporary vertical center guide** (high-contrast line) for the prototype.
